...
from telegram.ext import ApplicationBuilder, CommandHandler, ContextTypes, Application  # FIXED: Imported Application
...
# ====== FOREST SIGNAL STRATEGY ======
def get_5m_data(symbol):
    """Pull last 5 days of 5min candles and flatten column structures"""
    try:
        ...
        # FIXED: Flatten yfinance MultiIndex columns to single level
...
def check_forest_signal(pair_name, symbol):
    ...
    try:
        # FIXED: Core series data extraction explicitly enforced via .squeeze()
        ...
        if buy:
            return {
                "pair": pair_name, "direction": "BUY", "entry": price,
                "sl": round(price * 0.998, 4),
                "tp1": round(price * 1.004, 4),
                "tp2": round(price * 1.008, 4),
                "timeframe": "5M"
            }
        if sell:
            return {
                "pair": pair_name, "direction": "SELL", "entry": price,
                "sl": round(price * 1.002, 4),
                "tp1": round(price * 0.996, 4),
                "tp2": round(price * 0.992, 4),
                "timeframe": "5M"
            }
    except Exception as e:
    ...
    for pair_name, symbol in MAJOR_PAIRS.items():
        ...
        if sig:
            ...
            try:
                ...
                await asyncio.sleep(1)  # Throttling to honor API limits
            except Exception as e:
                logging.error(f"Failed to transmit Telegram message: {str(e)}")

async def auto_scanner(app: Application):
    # Wait for the system startup initialization sequence to complete safely
...
def main():
    ...
    # Secure background loop hooks inside framework lifecycle
    app.post_init = lambda app: asyncio.create_task(auto_scanner(app))
